...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with get_session() as session:
            existing_cols = {row[1] for row in session.execute(text("PRAGMA table_info(sites)"))}
            ...
            # The SQLAlchemy `location` default is Python-side, so rows created before this
            # column existed (or inserted via raw SQL) can still be NULL -- backfill them.
            loc_rows = session.execute(select(Site).where(Site.location.is_(None))).scalars().all()
            for site in loc_rows:
                site.location = "United States"
            if loc_rows:
                self.stdout.write(f"Backfilled location for {len(loc_rows)} row(s).")
