...
| `domain_lookups` | `DomainLookup` | domain, path, location, block | `domain_lookup_store` — every Domain Overview lookup, so a domain is paid for once rather than once a day |
...
| Keying a per-project decision on `site_url` | Many projects share one domain (18 premierstaff city projects). The sync guards did this twice: the in-flight check attached project B's fetch to project A's run (progress bar "succeeded", nothing fetched for B), and the SyncLog freshness check let A's sync mark all 17 siblings "fresh" for 24 h. Result: brand-new city projects permanently blank. If the answer differs per project, key it on `site_pk` (`RefreshRun.site_pk`), not the domain — see `_attach_or_report_sibling` / `project_scope_last_ran`. The **scheduler** had the same bug a third time (found on the live server 2026-09-01): `scheduling.last_run_at` and `run_scheduled_syncs` were domain-keyed and passed no `site_pk`, so any city's manual positions fetch reset the weekly clock for all 18 siblings and the scheduler never started positions on premierstaff.com at all. Now `LOCATION_SCOPED_MODULES` key their clock on `site_pk` and the command walks `active_projects()`. When you add a per-project decision, grep every path that makes it — the UI path, the API path AND the cron path |
...
| Trusting `llm_mentions/top_pages` to return only YOUR pages | It returns co-occurring pages from other domains too — a live call for `driphydration.com` came back containing `https://www.perfectb.com/...`, so a list headed "Your Most-Cited Pages" must not render them. **Filter when you READ, not when you write** (changed 2026-08-12): the connector used to drop them before storing, and combined with `items_list_limit: 10` that left a real project holding **four** rows out of a response it had already been billed for — the card showed four pages and looked like the crawl was broken. Every row is stored now (limit 100, the endpoint is priced per REQUEST plus ~$0.001/row, so a smaller limit saves almost nothing), and `llm_mentions_service.page_is_ours()` splits them into `topPages` / `coCitedPages` at read time. General rule: a WRITE-time filter throws away data you paid for and cannot be undone without re-buying it; a READ-time filter is free and reversible |
...
| Treating a 24h cache as persistence for a METERED lookup | Domain Overview held all three of its blocks in a Django cache and nothing else, so re-opening a URL the next morning bought it again. On `llm_mentions/search/live` that is a **$0.10 fixed fee per request** plus ~$0.001 per row (measured: limit 10 → $0.1100, limit 25 → $0.1250), which makes "the same request tomorrow" the most expensive habit a page can have — and makes *shrinking the limit* almost pointless, since 10 rows cost $0.11 and 100 cost $0.20. The lever is fewer REQUESTS: one row per (domain, path, location, block) in `domain_lookups`, read order **store → cache → network**, and only an explicit `refresh` re-buys. Two corollaries worth copying: cache/store on the coarsest key the API can actually be asked for (this endpoint rejects a path, so one domain call answers every page on it — keying per URL would have re-bought the domain for each page), and a PDF built with `allow_fetch=False` then reads a lookup of ANY age instead of coming back with empty sections after a day |
| Adding a store under code whose tests had no analytics-DB fixture | `test_domain_overview_questions.py` needed no temp DB because nothing in it touched the analytics database — until these blocks began persisting, at which point every test in it wrote into the developer's real `data/fusehealth.db` and stored rows leaked between tests *and between runs*. Six of them failed only when run alongside their neighbours. When you make previously pure code persist, `grep` its test modules for the §8 fixture before assuming they are isolated |
| Two connectors in one run buying the same API result | `dataforseo_serp` bought each keyword's top-30 SERP and kept only its own row; `dataforseo_serp_competitors` then bought the IDENTICAL SERP (same keyword, city, device, depth) to read the other 29. On the live account that duplicate was the largest line on the bill ($7.59 of $25 / 90 d) and would have grown 18× under per-city scheduling. `sync_engine._attach_run_context` gives every connector in a run ONE shared dict (`connector.run_shared`); a connector that pays for something a later connector needs publishes it there (`serp_tasks`), and the consumer reads it and posts nothing — falling back to its own purchase only when the producer did not run in this process. Before adding a connector that calls an endpoint another connector already calls in the same scope, ask what the first call already returned |
| Scoping the STORAGE of a per-project table without scoping its READERS | `tracked_competitors` was migrated to a `site_pk` key, the write path and the Settings read were updated, and tests were written — but four readers still called `get_tracked_competitors(site_id)` with no `site_pk`: `_get_competitor_map`, `_get_competitor_grid`, `backlinks_service` and the `dataforseo_serp_competitors` connector. Both `shared_queries` functions already HAD `site_pk` in scope; they just did not pass it. So three Premierstaff city projects each showed the other two's competitors, the Settings modal (which read scoped) disagreed with the Share of Voice panel on the same screen, and the connector spent DataForSEO credits capturing SERPs for sibling projects' competitors. The migration is not the fix — `grep` every caller of a function whose scope you just changed, and test through the CALLER, not the service |
| Falling back to auto-discovered or hardcoded values when a user has chosen none | `get_tracked_competitors` fell back to DataForSEO's `competitor_domains` (keyed by DOMAIN, so shared by every sibling project) and `_get_competitor_grid` then hardcoded `["linkedin.com", "instagram.com", "facebook.com", "youtube.com", "reddit.com"]` on top of that. A user saw youtube.com and indeed.com listed as their competitors in Share of Voice, next to a Settings modal showing four real ones, and correctly concluded the page was making things up. A suggestion is not a measurement: return the empty list and let the UI say "none chosen yet" — rule 3 covers invented *sets*, not just invented numbers |
| A "domain" normaliser that strips the scheme but not the PATH | `competitor_service._bare()` turned a pasted `https://premierstaff.com/event-staffing-agency-las-vegas` into exactly that string as a competitor domain, so the user's own landing pages appeared as their rivals. Use `pipeline/utils/site_ids.normalize_domain()` — the one canonicaliser (§3) — anywhere a human can paste a URL. Repair rows already written with `manage.py clean_competitor_domains --apply` (dry-run by default) |
