...
| Keying a per-project decision on `site_url` | Many projects share one domain (18 premierstaff city projects). The sync guards did this twice: the in-flight check attached project B's fetch to project A's run (progress bar "succeeded", nothing fetched for B), and the SyncLog freshness check let A's sync mark all 17 siblings "fresh" for 24 h. Result: brand-new city projects permanently blank. If the answer differs per project, key it on `site_pk` (`RefreshRun.site_pk`), not the domain — see `_attach_or_report_sibling` / `project_scope_last_ran` |
