...
def fetch_backlinks_payload(target_domain: str) -> dict:
    ...
    summary = _post("summary/live", {"target": domain, "include_subdomains": True})
    rdoms = _post("referring_domains/live", {"target": domain, "limit": 200,
                                             "order_by": ["rank,desc"], "include_subdomains": True}).get("items") or []
    anchors = _post("anchors/live", {"target": domain, "limit": 60, "order_by": ["backlinks,desc"]}).get("items") or []
    history = _post("history/live", {"target": domain}).get("items") or []
...
def refresh_backlinks(site_url: str, target_domain: str) -> dict:
    ...
    payload = fetch_backlinks_payload(target_domain)
